...
def Dissemble_encoded_string(encoded_string: str):
    # takes an encoded string and breaks it up into its (hopefiully seven components
...
class AIS_Data:
    ...
    _band = False  # base stations can command units to switch frequency if True unit can use any part of band
    _m22 = False  # Unit can accept channel assignment via message type 22
    _assigned = True  # Assigned mode flag False = autonomous, true = assigned mode
    _version = 0
    _IMO = 0
    _type = 0
    _d2bow = 0
    ...
    def set_AIS_Binary_Payload(self, value):
        if isinstance(value, int):
            ...
            raise RuntimeError(
                "Incorrect type not integer supplied to set AIS_Binary_Payload"
            )

    AIS_Binary_Payload = property(get_AIS_Binary_Payload, set_AIS_Binary_Payload)

    def get_AIS_Payload_ID(self):
        return self._Payload_ID

    AIS_Payload_ID = property(get_AIS_Payload_ID)
    ...
    def m_setup(self, Encoded_String: str):
        nr_items = 0
        # dictionary used to avoid a case statement in AIS_Data translates channel number
        ...
        try:
            # under c# discrete_items would be a string array, using pythin its a list
            discrete_items = Encoded_String.split(",")
            nr_items = int(len(discrete_items))
        except:
            print("Error Splitting AIS_DATA string", sys.exc_info()[0])
            # System.ArgumentException argEx = new System.ArgumentException("Error Splitting AIS_DATA string")
            raise
        ...
        for Data in discrete_items:
            ...
            if discrete_items[0] == "!AIVDM":
                if nr_items > 4:
                    ...
                    self._Frag_no = int(discrete_items[2])  # current fragment number
                    xx = discrete_items[3]  # message id for multifragment message
                    if len(xx) > 0:
                    ...
                    if self._channel in ch_numb_dict:
                        ...
                        raise RuntimeError("channel number not in range {A,B,1,2")
                    try:
                        self._payload = discrete_items[
                            5
                    ...
                    print(
                        "Insufficient fields in string " + Encoded_String,
                        sys.exc_info()[0],
                    )
                    raise

                self._Payload_ID = self.m_to_int(self._payload[0])  # payload type
                if diagnostic4:
                    print("in m_setup_Payload_ID = ", self._Payload_ID)
                self._binary_payload = self.create_binary_payload(
                    self._payload
                )  # binary form of payload
                if diagnostic4:
    ...
    def ROT(self):
        ...
        # to decode divide bcoded value by 4.733 then square. Returns rate in degrees per minute to three decimal places
        self._frot = float(0)
        # switch (_rot)
    ...
    def set_int_latitude(self, value):
        ...
        if isinstance(value, int):
            ...
            raise RuntimeError(
                "incorrect type {} in set_ini_latitude should be int".format(
                    type(value)
                )
            )
    ...
    def set_int_longitude(self, value):
        ...
        if isinstance(value, int):
            ...
            raise RuntimeError(
                "incorrect type {} in set_ini_longitufde should be int".format(
                    type(value)
                )
            )
    ...
    def set_COG(self, value):
        if isinstance(value, float):
            ...
            print(
                "in AISDATA setting COG got incorect type not int or float type = ",
                type(value),
            )
        self._cog = value
    ...
    def set_HDG(self, value):
        ...
        if isinstance(value, float):
            ...
            print(
                "Error setting heading incorrect type excpected int or float got ",
                type(value),
            )
    ...
    # it looks as though type 5 static data packets may not conform to standard (destination truncated
    ...
    def Type24PartNo(self):
        ...
        if self._Payload_ID == 24:
            ...
            if self.part < 2:
                ...
                raise NameError("Invalid Part No in Type 24 payload = " + str(part))
        else:
            return -1
    ...
    def ExtractString(self, startpos: int, blength: int) -> str:
        # grab data 6 bit byte by byte checking that the binary stream is not truncated from standard
        try:
            ...
            print(
                "Error in ExtractString startpos = "
                + str(startpos)
                + " blength = "
                + str(blength)
                + "binary_length = "
                + str(self._binary_length)
            )
    ...
    def Binary_Item(self, startpos: int, blength: int) -> int:
        # newer version of Binary_Item using the monster integer produced as _binary_payload
        ...
        _bitlength = self._binary_length
        # print('entering Binary_Item - number of bits in binary payload = ',_bitlength, '\r\n binary payload ', bin(self._binary_payload))
        # check that not going to overun the bitstream
        if (startpos + blength) > _bitlength:
            raise OverflowError(
                "The requested bit stream in Binary_Item will exceed size of binary payload\n\r "
                + "bitlength = ",
                _bitlength,
                " startpos = ",
                startpos,
        ...
        # determine where in bitstream the lsb is then shift the stream
        # into a temporary bit shifted so that lsb is rightmost
        _temp = int(0)
        lsb_pos = int(_bitlength - (startpos + blength))
        _temp = self._binary_payload >> lsb_pos
        # now mask the upper bits leaving only the required number of bits
        # done by and the temporary with (2**blength - 1) which produces ca string of blength ones
        # print('_temp prior masking ', bin(_temp))
        _temp = _temp & ((2 ** blength) - 1)
        # print('_temp after masking ', bin(_temp))
    ...
    def create_binary_payload(self, p_payload: str):
        # munging binary strings undedr python is a difficult exercise
        # now trying a method where I create an integer, shift 6 bits and add next nibble
        # the routines to extract bits from the binary payload will also need munging

        self._binary_payload = 1  # the integer to hold the binary string
        # inorder to get upper zerobits must preface the binary object with a one bit
        # this will need be taken care of in the function to extract bits
        # binary string has one more (msb) bit than calculated
        # print(' entering create binary payload ',self._binary_payload,'\r\n ',p_payload)
        self._binary_length = len(p_payload) * 6  # expected number of bits
        ...
        i = 0
        for xchar in p_payload:
            if i == len(p_payload):
                break
            # print ('in create binary payload ',len(p_payload), i)
            # iterate through the string payload masking to lower 6 bits
            # first shift the binary payload left 6 - for first entry 0 shifted 6 is still zero
            self._binary_payload = self._binary_payload * 2 ** 6
            # print('shifted binary payload =', bin(self._binary_payload))
            # nibble = self.m_to_int(p_payload[i]) & 0x3F  # ensures only 6 bits presented
            nibble = self.m_to_int(xchar) & 0x3F  # ensures only 6 bits presented

            # print(xchar, self.m_to_int(xchar) & 0x3F,bin(self.m_to_int(xchar) & 0x3F),bin(nibble))
            self._binary_payload = self._binary_payload + nibble
            # print('binary payload = ', bin(self._binary_payload), 'i= ', i)
            i += 1
    ...
    def m_to_int(self, parameter: str) -> int:
        ...
        if len(parameter) == 1:
            ...
            print(
                "multiple characters not yet handled in m_to_int\r\n", sys.exc_info()[0]
            )
